...
class GRUD(nn.Module):
    def __init__(self, input_size, cell_size, hidden_size, X_mean, output_last=False):
        ...
        print("creating gates of dimensions ")
        print(input_size + hidden_size + self.mask_size)
        print(hidden_size)
        ...
        self.ol = nn.Linear(input_size, 1) # output layer for classification task
    ...
    def step(self, x, x_last_obsv, x_mean, h, mask, delta):
        ...
        #TODO CHECK WHERE NAN COMES FROM AND CHECK EACH TERM!!!! maybe mean etc. 
        x = mask * x + (1 - mask) * ( delta_x * x_last_obsv + (1 - delta_x) * x_mean)  # TODO equation 5 in paper delta x = decay?
        ...
        # reducing dimensionality to one as classification problem
        h = self.ol(h)
    ...
    def forward(self, input):
        ...
        # no step size in our classification problem 
        
        for i in range(step_size):#TODO iterate through all time steps and store last hidden state??
            
            # print(X.shape)
            a = torch.squeeze(X[:, i:i + 1, :])  
            b = torch.squeeze(X_last_obsv[:, i:i + 1, :])  
            c = torch.squeeze(self.X_mean[:, i:i + 1, :])

            d = torch.squeeze(Mask[:, i:i + 1, :])  
            e = torch.squeeze(Delta[:, i:i + 1, :])  
